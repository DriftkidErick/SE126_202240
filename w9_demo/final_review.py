...
def swap(listname, position):
    ...
    temp = listname[position] #at the starting position
...
#1 - connect to a file and import its data into lists
with open("C:/Users/008004507/Desktop/SE126_202240/w9_demo/finalReview_text.csv") as csvfile:
    ...
    for rec in file:
        ...
        if int(rec[5]) == 1:
            ...
            #will catch any nums other than 1 or 2 (unexpected)
            color2.append("*ERROR*")
...
#bubble sort to order the ages! increasing numberic order
for i in range(0, num_rec - 1 ):

    for k in range(0, num_rec - 1): #k is just a random letter just dont use i cause its in the outter loop
...
#check the sort should be ordered in increasing age
print("\n\n{0:10}\t{1:15}\t{2:15}\t{3:3}\t{4:35}\t{5:3}\t{6:7}\t{7:7}".format('IdCode', "Last", "First", "Age", "Allegiance", "Num", "Color1", "Color2"))
...
#stores the youngest /oldest into vars to hang on to fro later
...
print("Average Age: \t{0:.2f}".format(ave_age))
...
while answer == "y":
    ...
    if userChoice == 2:
        ...
        print("\t\tBinary Search")
        #ask for search query
        search = input("Please enter the FRISTNAME you are searching for: ")
